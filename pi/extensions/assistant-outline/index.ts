...
export default function assistantOutlineExtension(pi: ExtensionAPI) {
  const openAssistantOutline = async (args: string, ctx: ExtensionContext) => {
    if (!ctx.hasUI) {
    ...
    const exportedSections = await ctx.ui.custom(
      (tui, theme, _kb, done) => {
        ...
        const openInEditor = async (buffer: string, missingMessage: string) => {
  ...
  pi.registerCommand(COMMAND_NAME, {
    ...
    handler: openAssistantOutline,
  });

  pi.registerShortcut("alt+o", {
    description: "Open assistant outline",
    handler: async (ctx) => {
      await openAssistantOutline("", ctx);
    },
  });
}
